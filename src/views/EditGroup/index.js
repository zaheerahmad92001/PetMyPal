...
import { HEADER, TEXT_DARK, TEXT_INPUT_LABEL, PLACE_HOLDER, BLUE_NEW, darkSky, PINK, grey, } from '../../constants/colors';
...
const { deleteOwnerGroup, updateOwnerGroupData, getPetOwnerGroups, createPetOwnerGroup, getMembers } = petMyPalGroupApiService;
...
class CreateEditGroup extends React.Component {
    ...
    createGroup = async (data) => {
        ...
        const response = await this.props.createPetOwnerGroup(this.props.token, formData);
        ...
        if (api_status == 200) {
            ...
            this.formRef.current?.setErrors({ CommunityName: 'Community name is taken. Please try another name' })
            this.setState({
                loader: false,
                isModal_Visible: true,
                infoText: 'Community Name Is Taken. Please Try Another Name',
    ...
    updateExistingGroup = async (values) => {
        ...
        const { coverImage, avatarImage, type, confirm } = this.state
        this.setState({ loader: true });
        const formData = new FormData();
        formData.append('server_key', server_key);
        ...
        const response = await this.props.updateOwnerGroupData(this.state.token, formData);
    ...
    renderButtons = (handleSubmit) => {
        ...
        if (_.isEmpty(this.state.group)) {
            jsx.push(<View style={{ width: '100%', alignItems: 'center' }}>
                <SkyBlueBtn
    ...
    getGroupMember = async () => {
        const { group_id, Memberlist } = this.state
        const type = 'get_members'
        ...
        return (
            <Container style={{ flex: 1 }}>
                ...
                {this.state.loading ? <LoaderWrapper><CustomLoader /></LoaderWrapper> :
                    <>
                        <PMPHeader
                            centerText={_.isEmpty(this.state.group) ? 'Create Community' : 'Edit Community'}
                        ...
                        <Content
                            contentContainerStyle={{ paddingBottom: _.isEmpty(this.state.group) ? wp(8) : 0 }}
                        >
                            {!_.isEmpty(this.state.group) &&
                                <ExistingGroup showDeleteModal={this.showDeleteModal}
                                    updateGroup={this.updateGroup} group={this.state} />
                            }
                            <Formik
                            ...
                            >
                                {({ handleSubmit, values, setErrors, errors }) => {
                                    ...
                                    return (
                                        <>
                                            ...
                                            <Label style={{ color: TEXT_INPUT_LABEL, fontSize: RFValue(14), marginLeft: wp(6), marginTop: wp(2), marginBottom: wp(1) }}>Pet Category</Label>
                                            <Field
                                                component={PetCategory}
                                                name='PetCategory'
                                                defaultValue={this.state.group?.category}
                                            />
                                            {/* <Divider
                                             style={{width:'90%',alignSelf:"center",borderBottomColor:0.1,borderBottomColor:grey}}
                                            /> */}
                                            <Label style={{ color: TEXT_INPUT_LABEL, fontSize: RFValue(14), marginLeft: wp(6), marginTop: wp(2) }}>
                                                About Community
                                            </Label>
                                            ...
                                            {_.isEmpty(this.state.group) ? null :
                                                <SkyBlueBtn
                                                    title={'Members'}
                                                    btnContainerStyle={styles.memberBtn}
                                                    onPress={() => {
                                                        // this.getGroupMember()
                                                        this.setState({
                        ...
                        {_.isEmpty(this.state.group) ? null :
                            <MemberModal
                                ...
                                isOwnGroup={this.props?.user?.user_data?.user_id}
